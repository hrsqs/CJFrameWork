...
@interface CJCatchException()
@property (assign, nonatomic) BOOL dismissed;
@end

@implementation CJCatchException
...
//    NSLog(@"%@", exceptionInfo);
    ...
    NSArray  *dirArr  = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES);
    ...
    if (isExistLogDir) {
        ...
        // 此处可扩展
        NSString *logPath = [logDir stringByAppendingString:[NSString stringWithFormat:@"/%@.txt", dateTime]];
...
void SignalHandler(int signal) {
    ...
    NSArray *callStack = [CJCatchException backtrace];
    [userInfo setObject:callStack forKey:UncaughtExceptionHandlerAddressesKey];
